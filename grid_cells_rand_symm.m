...
b=ones(1,N)*.001; %feedforward pinned at a point to hold bumps fixed. default .001
...
%     set(gcf, 'visible', 'off');
    ...
    %DYNAMICS START
    % reset
%     if mod(t,sum(reset_time))<reset_time(2)
%         %I=zeros(1, N);
%         %I=I(randperm(N));
%         %I=normrnd(0.76*threshold,0.1*threshold,[1 N]);
%         %noisesparsity=10;
%         b=ones(1,N)*.001;
%     else
%         %noisesparsity=1.5;
...
%     if burnedin
        ...
        if j>size(binnedspikes,1)
            binnedspikes=[binnedspikes; zeros(chunk,N)];
        end
        if phasetrack
...
N*size(binnedspikes,1)*binsize*dt*1000/sum(binnedspikes(:))
...
%save(sprintf('binnedspikes_unbinarized_unpinned_reset/sW %.4f.mat',sW),'binnedspikes','-v7.3');

figure;
imagesc(binnedspikes); colorbar;
title(sW)
