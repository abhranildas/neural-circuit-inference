...
b=[1,ones(1,N-1)]*.001; %feedforward pinned at a point to hold bumps fixed. Default amplitude =.001.
%b=ones(1,N)*.001;
...
for temp=11
    ...
    plotstyle='linear'; %linear, polar, 3D or none
    phasetrack=0;
    ...
    %     set(gcf, 'visible', 'off');
        ...
        %DYNAMICS START
        I=sW*s*W+b; %neuron inputs. The weight part is a simple matrix multiplication
        rate=32*max(I-threshold,0); %nonlinearity, r0=32 by default
        %rate=exp(I);
        rlist(t+1)=rate(2);
        %spike=random('Gamma',k,rate/k,1,N);
        spike=poissrnd(rate);
    ...
    %save(sprintf('binnedspikes_LNP_unpinned/sW %.4f.mat',sW),'binnedspikes','-v7.3');
    toc
end
